...
Normalización de la propiedad de destino (layer / layerMask)

- Objetivo: ofrecer una representación canónica y predecible para el campo destino de las luces
  (puede venir como nombre de layer o como bitmask numérico) y facilitar la edición/serialización.

- API relevante:

  - `LightRegistry.normalizeLayerTarget(target)` — convierte strings, números, arrays u
    objetos ({ name, bit }) a una forma canonical: `{ layer?: string, layerMask?: number }`.
  - `LightRegistry.setLayerResolver(resolver: (name: string) => number | undefined)` — permite
    registrar una función externa que mapea nombres de layer a su bitmask (por ejemplo la
    implementación de `Scene`). Cuando existe un resolver, `normalizeLayerTarget('player')`
    devolverá tanto `{ layer: 'player', layerMask: 0x0002 }`.
  - `LightingSystem.setScene(scene)` — helper para registrar automáticamente el resolver
    proveniente de `Scene` usando `scene.getLayer(name)?.bit`.

- Comportamiento y notas de integración:

  - Cuando se aplican propiedades vía `instance.setAllProperties` o `instance.setProperty`,
    las claves `layer` y `layerMask` se normalizan usando la ruta anterior y se aplican las
    dos formas cuando es posible (por ejemplo, al recibir un nombre y poder resolver su
    máscara se guardan ambos `layer` y `layerMask` en el estado interno).
  - Esto facilita que el inspector muestre el nombre de la capa mientras que el runtime
    usa masks numéricas para tests rápidos de bitmask en culling y composición.

- Ejemplo rápido de inicialización (dentro de un gestor o Engine):

```ts
// Asumiendo que tienes una Scene ya creada
const scene = new Scene("main");
const lightingManager = new GameLightingManager(scene); // constructor registra la escena

// Alternativamente, desde donde instancias LightingSystem directamente:
lightingSystem.setScene(scene);

// Ahora al setear una propiedad desde el editor:
lightInstance.setProperty("layer", "player");
// El registry normalizará y también rellenará layerMask si puede resolver 'player'
```

- Tests: añadimos tests unitarios que cubren `normalizeLayerTarget` con y sin resolver
  (string, number, array de numbers, array de names). Esto asegura roundtrips y evita
  errores de integración cuando la escena o el inspector interactúan con las luces.
